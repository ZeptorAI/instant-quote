...
export default async function AdminPage() {
  // The public demo is intentionally deployable without a persistent database.
  // In that mode, show an empty request feed instead of crashing the page.
  let rows: Awaited<ReturnType<typeof prisma.quoteRequest.findMany>> = [];
  try {
    if (process.env.DATABASE_URL) {
      rows = await prisma.quoteRequest.findMany({ orderBy: { createdAt: "desc" } });
    }
  } catch (error) {
    console.warn("[admin] Database unavailable; showing empty request feed.", error);
  }

  const [deal, rates] = await Promise.all([
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
          ...
          <StatCard label="Pipeline value" value={money(pipeline)} accent="green" hint="Σ range midpoints" />
          <StatCard label="Avg order qty" value={num(avgQty)} accent="cyan" />
          <StatCard label="Recurring buyers" value={num(recurringBuyers)} accent="amber" />
        </div>

        {deal ? (
          <AdminClient requests={requests} deal={deal} rates={rates} />
        ) : (
          <p className="mt-6 text-sm text-amber">No deal found.</p>
        )}
      </main>
    </div>
  );
...
function StatCard({
  ...
  const ring = { blue: "border-blue/30", green: "border-green/30", cyan: "border-cyan/30", amber: "border-amber/30" }[accent];
  const text = { blue: "text-blue", green: "text-green", cyan: "text-cyan", amber: "text-amber" }[accent];
  return (
    <div className={`card p-4 ${ring}`}>
      <div className="text-[11px] uppercase tracking-wider text-muted">{label}</div>
      <div className={`tnum mt-1 font-display text-2xl font-extrabold ${text}`}>{value}</div>
