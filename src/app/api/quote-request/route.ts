 ...
 *  2. recompute the estimate SERVER-SIDE (never trust client math)
 *  3. persist the QuoteRequest
 *  4. fire-and-forget forward the full payload + estimate to the webhook
 */
export async function POST(req: NextRequest) {
  ...
  if (!deal) {
    return NextResponse.json(
      { error: `Unknown deal: ${body.deal_id}` },
      { status: 404 }
    );
  }
  ...
  if (body.quantity < deal.moq) {
    return NextResponse.json(
      { error: `Quantity must be at least the MOQ of ${deal.moq}.` },
      { status: 422 }
    );
  }
  ...
  const restrictedHit = body.sales_channels.filter((c) =>
    deal.restrictedChannels.includes(c)
  );

  const ref = makeRef();

  // The exact payload shape the team already consumes.
  const payload = {
  ...
  const saved = await prisma.quoteRequest.create({
    data: {
      ...
      buyerName: body.buyer_name?.trim() || body.buyer_id,
      dealId: body.deal_id,
      salesChannels: JSON.stringify(body.sales_channels),
      quantity: body.quantity,
      managedByGws: body.logistics.managed_by_gws,
      destinationPostalCode: body.logistics.destination_postal_code,
      hasLoadingDock: site.has_loading_dock,
      hasForklift: site.has_forklift,
      requiresLiftgate: site.requires_liftgate,
      purchaseFrequency: body.purchase_frequency,
      budgetBand: body.budget_band,
      insurance: body.insurance,
      estLow: estimate.low,
      estHigh: estimate.high,
      perUnit: estimate.perUnit,
  ...
  // Fire-and-forget webhook forward. Never fail the user if the webhook is down.
  ...
  return NextResponse.json(
    {
      ...
      estHigh: estimate.high,
      perUnit: estimate.perUnit,
      perUnitLow: estimate.perUnitLow,
      perUnitHigh: estimate.perUnitHigh,
      restrictedHit,
    },
    { status: 201 }
  );
...
async function forwardToWebhook(payload: unknown) {
  ...
  if (!url || url.includes("your-uuid-here")) {
    console.warn(
      "[quote-request] QUOTE_WEBHOOK_URL not configured; skipping forward."
    );
    return;
  }
  try {
    ...
    if (!res.ok) {
      console.error(`[quote-request] webhook responded ${res.status}`);
    }
  } catch (err) {
    console.error("[quote-request] webhook forward failed:", err);
  }
}
